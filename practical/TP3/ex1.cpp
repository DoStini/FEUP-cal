 ...
 * i and j iterate over the vector
 * Variant: n - i -> since i increases and n is constant, n - i is decrescent, so n - i will reach 0.
 * Invariant: res.dmin is the minimum distance
 *              In the beggining there is no comparision made, so the value of the distance is MAX_DOUBLE, meaning there are not 2 points yet
 *              If there are at least 2 points, res.dmin contains the smalles distance until the last iteration, because for each 2 points, their distance is verified, compared and replaced if smaller
 *              After the last iteration, res.dmin contains the minimum distance until now
 *
 * @param vp
 * @return
 */
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); ++i)
        for (int j = i+1; j < vp.size(); ++j)
            if ((val = vp[i].distance(vp[j])) < res.dmin || res.dmin == MAX_DOUBLE)
                res = Result(val, vp[i], vp[j]);
    return res;
}
...
Result recCallDC(std::vector<Point> &vp, size_t i, size_t j) {
    if (j-i == 2)
        return Result(vp[i].distance(vp[i+1]), vp[i], vp[i+1]);
    else if (j-i < 2) {
        Result res; res.dmin = MAX_DOUBLE;
        return res;
    }
    size_t split = (j+i)/2;
    Result rl = recCallDC(vp, i, split);
    Result rr = recCallDC(vp, split, j);

    Result res = rl.dmin < rr.dmin ? rl : rr;

    Result splres;
    double xmid = (vp[split].x + vp[split-1].x)/2;
    double d1 = xmid - res.dmin, d2 = xmid + res.dmin,  val;

    for (int k = split-1; k > 0; k--)
        for (int l = split; l < j; ++l) {
            if (d1 > vp[k].x || d2 < vp[l].x)
                break;
            if ((val = vp[k].distance(vp[l])) < splres.dmin) {
                splres = Result(val, vp[k], vp[l]);
                d1 = xmid - splres.dmin, d2 = xmid + splres.dmin;
            }
        }

    if (splres.dmin < res.dmin)
        return splres;

    return res;
}

void recCallDCMT(std::vector<Point> &vp, Result &result, size_t i, size_t j, int &threads) {
    if (j-i == 2) {
        result = Result(vp[i].distance(vp[i+1]), vp[i], vp[i+1]);
        return;
    }

    else if (j-i < 2) {
        result.dmin = MAX_DOUBLE;
        return;
    }
    size_t split = (j+i)/2;
    ...
    if (threads > 0) {
        threads--;
        std::thread t([&]() { recCallDCMT(vp, rl, i, split, threads); });
        recCallDCMT(vp, rr, split, j, threads);
        t.join();
    } else {
        recCallDCMT(vp, rl, i, split, threads);
        recCallDCMT(vp, rr, split, j, threads);
    }

    result = rl.dmin < rr.dmin ? rl : rr;

    Result splres;
    double xmid = (vp[split].x + vp[split-1].x)/2;
    double d1 = xmid - result.dmin, d2 = xmid + result.dmin,  val;
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    ...
    sortByX(vp, 0, vp.size()-1);
    int th = numThreads;
    recCallDCMT(vp, res,0, vp.size(), th);
    return res;
}
...
void testNearestPoints(NP_FUNC func, std::string alg) {
    ...
    (testNPFile("Pontos32k", 1.0, func, alg));

    testNPFile("Pontos64k", 1.0, func, alg);
    (testNPFile("Pontos128k", 0.0, func, alg));
    (testNPRand(0x40000, "Pontos256k", 1.0, func, alg));
    testNPRand(0x80000, "Pontos512k",  1.0, func, alg);
    testNPRand(0x100000, "Pontos1M",  1.0, func, alg);
    testNPRand(0x200000, "Pontos2M",  1.0, func, alg);
}
/*
...
TEST(TP3_Ex1, testNP_DC_12Threads) {
    setNumThreads(12);
    testNearestPoints(nearestPoints_DC_MT, "Divide and conquer with 8 threads");
}
